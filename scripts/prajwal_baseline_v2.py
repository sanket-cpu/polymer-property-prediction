...
# Number of conformers embedded per molecule for the 3D-conformer block
# (see conformer_3d_feats). A single ETKDG embed is a noisy sample of a
# flexible dimer backbone's conformational ensemble -- ambient thermal
# motion means the "true" dipole/shape isn't one geometry, it's a
# distribution, and one embed picks an arbitrary point on that
# distribution. Averaging descriptors across N_CONFORMERS independently-
# seeded embeds is a direct variance-reduction fix to that specific noise
# source, on exactly the block that's already gated to the polarizability-
# driven targets (eps/ei/nc) this matters most for. Cost scales ~linearly
# with this knob (N_CONFORMERS embeds + MMFF optimizations per molecule
# instead of 1) -- shrink this first, alongside N_OPTUNA_TRIALS/BAG_SEEDS,
# if a run is running long.
N_CONFORMERS = 8
N_SPLITS = 5
N_REPEATS_SMALL = 3
...
# ---------------------------------------------------------------------------
# 3D-conformer descriptor block (v3: multi-conformer, see module docstring)
# ---------------------------------------------------------------------------
_D3D_BASE_COLS = [
    'd3d_dipole', 'd3d_radius_of_gyration', 'd3d_asphericity',
    'd3d_eccentricity', 'd3d_spherocity', 'd3d_npr1', 'd3d_npr2',
]
# _std columns carry cross-conformer spread of each descriptor -- a
# molecule whose shape/dipole vary a lot across conformers (a floppy
# backbone) is telling us something a single mean value throws away,
# distinct signal from the mean itself.
_D3D_DEFAULTS = {c: np.nan for c in _D3D_BASE_COLS}
_D3D_DEFAULTS.update({f'{c}_std': np.nan for c in _D3D_BASE_COLS})


def conformer_3d_feats(smiles, n_confs=N_CONFORMERS):
    """3D-conformer descriptors, averaged across n_confs independently-
    embedded conformers: dipole moment magnitude + RDKit's Descriptors3D
    shape descriptors, off ETKDGv3-embedded + MMFF-optimized conformers of
    the *dimer* (reuses _make_dimer_mol exactly as built for the
    dimer-delta features, same reasoning as before -- a raw wildcard atom
    has no sane geometry to embed, but _make_dimer_mol already resolves
    both attachment points). D3D_TARGETS only (eps/ei/nc), called directly
    from main() on that subset of rows.

    Multi-conformer, not single-embed: a flexible dimer backbone has many
    reasonably low-energy geometries, and dipole moment/shape descriptors
    both vary meaningfully across them -- a single ETKDG embed is a noisy
    sample of that ensemble, not a stable read on the molecule. Embeds
    n_confs conformers (EmbedMultipleConfs, one RDKit call, internally
    seeded off RANDOM_STATE so the whole batch is still deterministic),
    MMFF-optimizes each, keeps only the ones that actually converged, and
    reports both the per-descriptor mean and std across surviving
    conformers -- the mean is the variance-reduced replacement for the old
    single value, the std is new signal (how conformationally flexible the
    molecule's shape/dipole actually are).

    Every failure point (dimer construction, embedding, MMFF setup, zero
    converged conformers) falls back to NaN for every column rather than
    raising -- same contract as before, just checked once for the whole
    batch instead of once for a single conformer. A handful of embed
    failures on unusual topologies (same ring-topology edge cases
    _make_dimer_mol already tolerates for the 2D dimer-delta features) are
    expected and must never crash the whole run over one bad molecule."""
    dimer = _make_dimer_mol(smiles)
    if dimer is None:
    ...
    try:
        ...
        # (converged_flag, energy) per conformer, in conf_ids order;
        # converged_flag == 0 means that conformer's MMFF optimization
        # succeeded -- non-zero (or an outright exception) means discard
        # just that conformer, not the whole molecule.
        try:
            opt_results = AllChem.MMFFOptimizeMoleculeConfs(mol_h, maxIters=500)
        except Exception:
            return dict(_D3D_DEFAULTS)
        good_ids = [cid for cid, (conv, _e) in zip(conf_ids, opt_results) if conv == 0]
        if not good_ids:
            return dict(_D3D_DEFAULTS)
        ...
        for cid in good_ids:
            try:
                ...
                for atom in mol_h.GetAtoms():
                    ...
                    dipole_vec += mmff_props.GetMMFFPartialCharge(idx) * np.array(
                        [pos.x, pos.y, pos.z])
                per_conf_rows.append({
                    'd3d_dipole': float(np.linalg.norm(dipole_vec)),
                    'd3d_radius_of_gyration': Descriptors3D.RadiusOfGyration(mol_h, confId=cid),
                    'd3d_asphericity': Descriptors3D.Asphericity(mol_h, confId=cid),
                    'd3d_eccentricity': Descriptors3D.Eccentricity(mol_h, confId=cid),
                    'd3d_spherocity': Descriptors3D.SpherocityIndex(mol_h, confId=cid),
                    'd3d_npr1': Descriptors3D.NPR1(mol_h, confId=cid),
                    'd3d_npr2': Descriptors3D.NPR2(mol_h, confId=cid),
                })
            except Exception:
                # one bad conformer (rare geometric degenerate case) is
                # dropped, not fatal to the row -- same tolerance as the
                # embed/MMFF-optimize stages above
                continue

        if not per_conf_rows:
            return dict(_D3D_DEFAULTS)

        conf_df = pd.DataFrame(per_conf_rows)
        means = conf_df.mean().to_dict()
        stds = conf_df.std(ddof=0).fillna(0.0).to_dict()  # single-survivor -> std 0, not NaN
        return {**means, **{f'{k}_std': v for k, v in stds.items()}}
    except Exception:
        return dict(_D3D_DEFAULTS)
...
def main():
    ...
    # dipole moment + RDKit Descriptors3D shape descriptors, averaged
    # across N_CONFORMERS conformers (v3 -- see module docstring). Real
    # per-molecule wall-clock cost (embedding + force-field optimization
    # x N_CONFORMERS), so computed only for these 3 targets' own rows --
    # unlike every feature block above, which is computed once for the
    # whole train_valid frame and shared across all 7 targets. A pure
    ...
    print("\n" + "=" * 100)
    print(f"3D-conformer descriptor block (multi-conformer, n={N_CONFORMERS}) -- "
          f"D3D_TARGETS only ({sorted(D3D_TARGETS)})")
    print("=" * 100)
